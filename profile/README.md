...
> “걸을수록, 지구가 가벼워집니다.”
Penglobe는 일상의 작은 실천을 데이터와 보상, 감성적 피드백으로 연결하는 환경 금융 플랫폼입니다.

사용자의 걸음과 식습관 데이터를 분석해 탄소 절감량을 정량적으로 시각화합니다. 특히 ‘빙하가 회복되고 펭귄 이파와 토리가 가까워지는 홈 화면’을 통해, 개인의 행동이 지구에 미치는 긍정적 변화를 직관적이고 감성적으로 경험하도록 설계했습니다.

AI 기반 식단 분석, 이동 탄소 계산, 공동체 랭킹 등 게이미피케이션 요소를 통해 환경적 가치를 수치와 스토리로 체감하게 하며, 획득한 리워드(얼음)는 친환경 소비와 기부로 이어집니다. Penglobe는 재미·데이터·금융을 결합하여, 일상의 실천이 곧 지구를 회복시키는 선순환 생태계를 구축합니다.
